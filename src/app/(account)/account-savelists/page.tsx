 ...
 */
// i18n-keys: web:myMoves.empty.filtered.pending.title, web:myMoves.empty.filtered.inProgress.title
// i18n-keys: web:myMoves.empty.filtered.completed.title, web:myMoves.empty.filtered.cancelled.title
...
function MyMovesContent() {
  ...
  return (
    <div>
      ...
      <h1 className="text-3xl font-semibold">{t('web:myMoves.title')}</h1>
      <p className="mt-2 text-neutral-500 dark:text-neutral-400">
        {t('web:myMoves.subtitle')}
      </p>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100 mb-2">
            {activeTab
              ? t(`web:myMoves.empty.filtered.${STATUS_LABEL_KEY[activeTab]}.title`)
              : t('web:myMoves.empty.title')}
          </h3>
          <p className="text-neutral-500 dark:text-neutral-400 mb-6 max-w-sm">
            {activeTab
              ? t('moves:filter.empty.helper')
              : t('web:myMoves.empty.subtitle')}
          </p>
          {!activeTab && (
            <Link
              href="/"
